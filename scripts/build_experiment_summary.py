...
def collect_rows() -> list[dict[str, str]]:
    ...
    rows.append(manual_row(
        model_family="DenseNet-121",
        run_timestamp="2026-07-30 15:45:03.830205",
        timezone="UTC",
        record_type="validation production-ROI robustness fine-tune",
        status="completed; not promoted",
        short_description="Production YOLO ROI robustness fine-tune",
        purpose="Test whether small YOLO ROI expansion and centre-shift variation reduces border-dependent Grad-CAM.",
        decision="Do not replace the paired-view production checkpoint: metrics improved but aggregate CAM border energy increased.",
        known_issue="CAM energy masks are broad geometry heuristics, not expert lesion annotations.",
        source_report="docs/AI_REPORT.md",
        notebook_archive="notebooks/experiments/densenet121/roi/dense_net_121_production_roi_robustness.ipynb",
        checkpoint_directory="Google Drive: Models/densenet121_production_roi_robustness/2026-07-30_15-45-03_830205_UTC",
        architecture="timm_densenet121_linear_gradcam",
        initialization="paired-view DenseNet-121 CE checkpoint",
        input_size="384x384",
        roi_and_crop_policy="YOLO box; train random expansion 1.10-1.20 and shift up to 5%; validation fixed 1.15 square; external black padding only",
        image_processing_full="LAB CLAHE 1.25; SquarePad; Resize 384; ImageNet normalization",
        training_augmentation_full="Horizontal flip p=0.50; rotation +/-5; brightness/contrast 0.08; RandomErasing p=0.10",
        validation_test_processing="Fixed production ROI and deterministic preprocessing; test set not opened",
        laterality_policy="Natural orientation; no deterministic mirroring",
        sampler="inverse-frequency WeightedRandomSampler",
        pipeline="five-epoch full fine-tune",
        training_stages="full model fine-tune",
        epochs="5",
        selected_epoch="4",
        batch_size="48",
        workers_gpu="2 / CUDA",
        ...
        seed="42",
        checkpoint_selection="0.55 QWK + 0.30 macro F1 + 0.15 macro AP",
        heatmap_method="Grad-CAM at DenseNet features.norm5",
        heatmap_resolution="12x12 upsampled to 384x384",
        dataset_split="5,778 train / 826 validation; 227 deterministic CAM audit cases; test not opened",
        validation_accuracy="0.6186",
        validation_qwk="0.7650",
        validation_macro_f1="0.6451",
        validation_average_precision="0.6756",
        validation_selection_score="0.7156",
        best_validation_epoch="4",
        cam_cases="227",
        cam_joint_energy="0.8505",
        cam_border_energy="0.0844",
        cam_peak_inside_rate="1.0000",
        cam_notes="Baseline: joint energy 0.8648, border energy 0.0761, peak-inside 0.9956. Candidate did not meet CAM promotion rule.",
        source_file="notebook inline outputs",
        record_origin="executed_notebook",
    ))
