...
class KneeOAPipeline:
    ...
    def __init__(self) -> None:
        ...
        # Load each component model and its checkpoint, storing the model and metadata.
        for component_name in component_names:
            config = self.component_config[component_name]
            checkpoint_path = getattr(settings, config["checkpoint_setting"])
            model, metadata = self._load_component(
            ...
            component_metadata[component_name] = metadata
        ...
        if self.model_mode == "ensemble":
            ...
            self.checkpoint_metadata = {
                ...
                "epoch": {
                    name: value["epoch"]
                    for name, value in component_metadata.items()
                },
                "loss_type": "ce",
                "validation_metrics": {
            ...
            self.checkpoint_metadata = component_metadata[component_names[0]]
    ...
    def _select_heatmap_component(
        self,
        component_probabilities: dict[str, torch.Tensor],
        predicted_class: int,
    ) -> str:
    ...
    def predict(self, image_bytes: bytes, knee_side: str = "unknown") -> dict:
        ...
        input_tensor, processed_image, _ = (
