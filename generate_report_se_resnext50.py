...
notebook_path = '/home/viet/Capstone/ml/notebooks/se_resnext50_32x4d.ipynb'
report_dir = '/home/viet/Capstone/ml/docs/report/se_resnext50'
...
model_name = config.get('model_name', 'seresnext50_32x4d')
img_size = config.get('img_size', '310')
pipeline = config.get('training_pipeline', 'paper_2stage')
epochs = config.get('stage1_epochs', '2') + " + " + config.get('stage2_epochs', '20')
sampler = config.get('use_balanced_sampler', 'True')
min_aug = config.get('use_minority_aug', 'False')
loss = config.get('loss_stage2', 'ce')
    ...
    if cell.get('cell_type') == 'code':
        ...
        for out in outputs:
            if 'data' in out and 'image/png' in out['data']:
                ...
                elif 'show_gradcam' in source or 'GradCAM' in source or 'Grad-CAM' in source:
                    img_type = "gradcam"
                    group_key = "Gradcam"
...
if actual_epochs == 0:
    try:
        actual_epochs = int(config.get('stage1_epochs', '2')) + int(config.get('stage2_epochs', '20'))
    except Exception:
        actual_epochs = 22
...
markdown = ""
if write_header:
    markdown += "# SE-ResNeXt-50 Training Execution Log\n"
    markdown += "This file automatically logs training runs, hyperparameters, metrics, and visualization plots.\n\n"

run_desc = "Paper 2-Stage Training" if pipeline == "paper_2stage" else "Standard Training Strategy"
markdown += f"## Run: {human_time_str} ({model_name.upper()} - {run_desc})\n"

# Summary Section
markdown += "### Summary\n"
markdown += f"This run successfully trained an {model_name} model in {pipeline} mode for {actual_epochs} epochs using {loss_name} loss. "
if metrics['QWK Score'] != "N/A":
    markdown += f"With the paper's 2-stage transfer learning and 5-crop Test-Time Augmentation (TTA), the model achieved a final test Accuracy of {metrics['Accuracy']} and a Quadratic Weighted Kappa (QWK) score of {metrics['QWK Score']}."
else:
...
try:
    epochs_val = int(config.get('stage1_epochs', 2)) + int(config.get('stage2_epochs', 20))
except Exception:
    epochs_val = 22

if actual_epochs < epochs_val:
    markdown += f"* **Early Stopping Triggered:** The model training stopped early at **Epoch {actual_epochs}** due to early stopping, showing that the regularization successfully prevented validation loss from continuing to rise.\n"
else:
    markdown += f"* **Full Training Completed:** The model completed all {epochs} epochs of paper 2-stage training.\n"
...
markdown += "* **Joint Space Targeting:** The Grad-CAM heatmap reveals that the model is successfully targeting the tibiofemoral joint space line and marginal osteophytes, matching the target patterns detailed in the Aleksei Tiulpin paper.\n\n"
...
notebook_copy_name = f"{timestamp_str}_{model_name}_{loss}.ipynb"
