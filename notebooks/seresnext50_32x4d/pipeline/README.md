# SE-ResNeXt-50 32x4d Pipeline

Three notebooks that take SE-ResNeXt-50 from ImageNet weights to a checkpoint the API can serve.
...
|---|---|---|---|---|
| 1 | `01_train_original.ipynb` | Published crops, 384×384 | `Models/seresnext50_32x4d_original/<timestamp>/` | ~45 min (12 epochs) |
| 2 | `02_train_paired_roi.ipynb` | 50% published / 50% YOLO ROI | `Models/seresnext50_32x4d_paired_roi/<timestamp>/` | ~20 min (5 epochs) |
| 3 | `03_evaluate_roi_test.ipynb` | Nothing — evaluation only | `Models/seresnext50_32x4d_evaluation/<timestamp>/` | ~5 min |
...
The ROI folder keeps its `densenet121_…` name for historical reasons — it holds plain YOLO crops and
is shared by **both** model pipelines. It is produced by
`notebooks/datasets/01_prepare_original_and_yolo_roi_datasets.ipynb`.
...
3. Only then point `SE_RESNEXT_CHECKPOINT_PATH` in the environment file at the new checkpoint.
