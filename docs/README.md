...
| "Show me every run you did" | [report/summary.md](report/summary.md) — all 83 configurations, grouped |
...
| [report/summary.md](report/summary.md) | **Start here** — all 83 configurations, grouped by kind, production artifacts first |
| [report/report.csv](report/report.csv) | Full record — 83 rows x 41 columns, one row per configuration; regenerate the summary with `build_summary.py` |
...
Rows whose `record_type` is `orphan_checkpoint` are checkpoints that exist on disk but that **no**
notebook trains or evaluates; their figures come from the checkpoint's own stored metadata and are
labelled as such. Rows marked `template_not_executed`, `aborted_run`, `incomplete_run`, or
`stale_outputs` deliberately carry no metrics.
